...
| Tela | O que é |
|---|---|
| **saldos** | Grade por mês: uma linha por dia, colunas `entradas · saídas · diários · economias · cartão · saldos`. Saldo é acumulado dia a dia e atravessa os meses. Célula vermelha quando o saldo é negativo, mais escura quanto mais negativo; positivo é um termômetro de "quantos meses eu aguento": amarelo cobre menos de 1 mês do custo de vida (média de saídas + cartão + diários dos últimos 3 meses com gastos, diário nunca abaixo da previsão × dias), verde claro de 1 a 2 meses, verde forte acima de 2. Sem custo conhecido, positivo é verde claro. Dia de hoje destacado; fins de semana sombreados. Cada célula tem "adicionar movimentação" e "ver movimentações". Cada dia tem um "check-in" (marcar o dia como conferido). Colunas podem ser encolhidas. Rodapé com o total do mês por coluna. Seletor de período (mês a mês ou ano a ano). |
