...
(() => {
  ...
  function renderMeses() {
    ...
    const custo = P.custoDeVida(EC, +hj.slice(0, 4), +hj.slice(5, 7)); // referência do termômetro: quantos meses de custo de vida o saldo cobre
    cont.innerHTML = resumos.map(({ a, m, r }) => {
      ...
      const linhas = r.dias.map(d => {
        ...
        return `<tr class="${cls}" data-linha="${d.data}"><td class="dia"><button type="button" data-checkin="${d.data}" aria-label="Check-in do dia ${d.dia}" aria-pressed="${d.checkin}">${d.dia}</button></td>${cels}<td class="saldo ${P.faixaSaldo(d.saldo, custo)}" style="--f:${f}"><button type="button" data-ver="${d.data}" data-tipo="" aria-label="Ver movimentações do dia ${d.dia}" style="width:100%;text-align:right">${P.fmtBRL(d.saldo)}</button></td></tr>`;
      }).join('');
